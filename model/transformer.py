...
class Transformer(nn.Module):
    ...
    def __init__(self, src_vocab_size, tgt_vocab_size, num_layers, num_heads, max_seq_length, d_model, d_ff, drop_prob):
        ...
        # self.encoder = nn.ModuleList([Encoder(num_heads, d_model, d_ff, dropout) for _ in range(num_layers)])
        # self.decoder = nn.ModuleList([Decoder(num_heads, d_model, d_ff, dropout) for _ in range(num_layers)])
    ...
    def forward(self, src, tgt):
        ...
        x = self.positional_encoding(src_embedding)
        return x